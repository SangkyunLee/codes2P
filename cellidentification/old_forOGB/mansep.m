...
uicontrol('Style', 'pushbutton', 'String', 'CLOSE','Position', [20 20 50 40],'Callback','close(hFig)'); 
...
for iclst=1:nofig
    ...
    set(hImg,'ButtonDownFcn',{@ButtonDown1,guidata(hImg),hFigs(iclst),clstimg,inx_auto_boundary,iclst});
     uicontrol('Style', 'pushbutton', 'String', 'Add a ROI','Position', [20 20 50 40],'Callback',{@Addaclst,guidata(hImg),iclst});   
end
...
% % close(hFig);
function closewin(hObject,eventdata, handles)
    close(hObject);

function Addaclst(hObject,eventdata, handles,iclst)
    global clststruct;
...
if strcmp(get(hfig,'SelectionType'),'normal')
    ...
    clststruct.sel_boundvox{iclst} = [clststruct.sel_boundvox{iclst} inx_voxel]; 
