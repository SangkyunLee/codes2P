function plot_roi_imgseqs0(mainpath, fnpattern, numdata,ROI)
% work with old prairie view software before 
%
% mainpath = 'D:\data_2photon\140103 BR\RF_140103BR_S1_20140206\data'
% fnpattern = 'TSeries-02062014-1233-001';
% numdata = [1:22];

fn_MC=cell(length(numdata),1);
data_subpath = cell(1,max(numdata));
data_Nseg=zeros(1, length(numdata));
    ...
    for ifl= 1 : length(flist)
        if ~flist(ifl).isdir
            if strcmp(flist(ifl).name(end-3:end),'.tif') %% && isempty(strfind(flist(ifl).name,'Voltage'))            
                ...
                fn1= sprintf(['MC_' data_subpath{idata} '_Cycle00001_CurrentSettings_Ch2.zip']);
...
if any(data_Nseg-1)>0
    error('this function works only with single segment');
end
% M.data.ROI = newROI;
for idata = 1 : length(numdata)  
    
    fnfig=sprintf('initialROI%d.jpg',numdata(idata));
    subpath=sprintf([fnpattern '-%03d'],numdata(idata));
    fn=sprintf(['AVG_MC_' fnpattern '-%03d_Cycle00001_CurrentSettings_Ch2.tif'],numdata(idata));
    finfo=fullfile(mainpath,subpath,fn);
    if ~exist(finfo,'file')
        fn=sprintf(['MC_' fnpattern '-%03d_Cycle00001_CurrentSettings_Ch2.tif'],numdata(idata));
        finfo=fullfile(mainpath,subpath,fn);
        bAVG = false;
    else
        bAVG = true;
    ...
    img1 = mean(Img,3);
    if ~bAVG
        avgfn = sprintf(['AVG_MC_' fnpattern '-%03d_Cycle00001_CurrentSettings_Ch2.tif'],numdata(idata));
