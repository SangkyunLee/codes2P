function [output Greg] = dftregistration(buf1ft,buf2ft,usfac, bgpu)
% bgpu mode is added by Sangkyun Lee 2015-07-21
...
if exist('bgpu')~=1, bgpu=false; end
...
% Partial-pixel shift
    ...
    if bgpu
        ...
        nlarge=n*2;
        CC=zeros(mlarge,nlarge);
        tmp =  gather(fftshift(buf1ft).*conj(fftshift(buf2ft)));
        CC(m+1-fix(m/2):m+1+fix((m-1)/2),n+1-fix(n/2):n+1+fix((n-1)/2)) = tmp;

        % Compute crosscorrelation and locate the peak 
        CC = gather(ifft2(gpuArray(ifftshift(CC)))); % Calculate cross-correlation
        [max1,loc1] = max(CC);
        [max2,loc2] = max(max1);
        ...
        % If upsampling = 2, no additional pixel shift refinement
            ...
            rg00 = gather(sum(sum( buf1ft.*conj(buf1ft) ))/m/n);
            rf00 = gather(sum(sum( buf2ft.*conj(buf2ft) ))/m/n);
        end
