function hImg = overlayImage(img01, cci, hax,mode,rangeVaules)
% cci: cluster image 
% img: raw image 3d image
% hax: axes handle in which image is drawn
% rangeValues added by Sangkyun Lee 04-28-2015

img01 = (img01- min(img01(:)))/(max(img01(:))-min(img01(:)));
...
if max(clustmap)==min(clustmap)
    ...
    if nargin>4,
        a = (clustmap-(rangeVaules(1)))/(rangeVaules(2)-rangeVaules(1)); 
    else
