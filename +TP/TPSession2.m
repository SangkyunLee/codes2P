classdef TPSession2 
    ...
    % This is a simple modification of TPsession, in collect_trialresp_singlescan
    % and in collect_trialMresp
    properties
...
%             self.eye = TP.detect_slpup(self.eye,par.puppar); % detect small and large pupil
        ...
        function [X, seltrial, timeF] = collect_trialresp_singlescan(self, dtype, inx_scan, pre_stimtime_ms, motionthr, beye)
            % function [X, Xc] = collect_trialresp_singlescan(self, dtype,inx_scan, pre_stimtime_ms, motionthr, beye)
        ...
        function [MX, trialinxs, tf] = collect_trialMresp(self, dtype, scanlist, twins ,motionthr, beye)
            % function collect_trialMresp(self, dtype, scanlist, twins ,motionthr,beye)
            ...
            for inx_scan0 = 1:ns
                ...
                [X,  seltrial, tf] = collect_trialresp_singlescan(self, dtype,inx_scan, pre_stimtime_ms, motionthr, beye);
              
                
                for iwin = 1: nwin
                    inxframes = (tf>twins(1,iwin) & tf<=twins(2,iwin));  
                                       
                    mX= squeeze(mean(X{1}(:,inxframes,:),2));                                                                              
                    MX{inx_scan0,iwin} = mX;
