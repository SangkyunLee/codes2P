function [n a history]=getspikes_testextraregressor(Y,opts)
...
for iter=1:MAX_ITER
    ...
    for ichunk = 1 : nchunk     
        ...
        if nR>0
            Usub = U(tinx1:tinx2,:);
            scale = sqrt(sum(Usub.^2,1))/norm(C1(:,1));        
            C1 = [bsxfun(@times,C1,scale) Usub];
            %C1 = [C1 bsxfun(@times,Usub,scale)];
        end
        ...
        Cnpart = C1*npart;        
        if nR>0
            CnRpart = C1(:,end-nR+1:end)*npart(end-nR+1:end);
            npart = npart(1:end-nR);            
        end
        
        if nchunk == 1
            n(tinx1:tinx2) = npart;
            Cn(tinx1:tinx2,1) = Cnpart;  
            CnR(tinx1:tinx2,1) = CnRpart;  
